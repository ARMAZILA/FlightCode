...
uint32_t lsm303dlhc_irq1_count = 0;
uint32_t lsm303dlhc_irq15_count = 0;

// ACCEL_INT1 - PC4
// MAG_DRDY   - PC5

/**
 * @brief  This function handles ACCEL_INT1 External lines interrupt request.
 * @param  None
 * @retval None
 */
void ACCEL_INT1_IRQHandler(void)
{
	lsm303dlhc_irq1_count++;

	/* Clear the ACCEL_INT1 EXTI line pending bit */
	EXTI_ClearITPendingBit(ACCEL_INT1_EXTI_LINE);
}

/**
 * @brief  This function handles MAG_DRDY External lines interrupt request.
 * @param  None
 * @retval None
 */
void MAG_DRDY_IRQHandler(void)
{
	lsm303dlhc_irq15_count++;

	/* Clear the MAG_DDRDY EXTI line pending bit */
	EXTI_ClearITPendingBit(MAG_DRDY_EXTI_LINE);
}

static void lsm303dlhcIrqConfig(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
	EXTI_InitTypeDef EXTI_InitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;

	/* Configure ACCEL_INT1 pin as input floating */
	GPIO_InitStructure.GPIO_Pin = ACCEL_INT1_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(ACCEL_INT1_GPIO_PORT, &GPIO_InitStructure);

	/* Connect EXTI Line to ACCEL_INT1 pin */
	GPIO_EXTILineConfig(ACCEL_INT1_EXTI_PORT_SRC, ACCEL_INT1_EXTI_PIN_SRC);

	/* Configure ACCEL_INT1 EXTI line */
	EXTI_InitStructure.EXTI_Line = ACCEL_INT1_EXTI_LINE;
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
	EXTI_Init(&EXTI_InitStructure);

	/* Enable and set ACCEL_INT1 EXTI Interrupt to the lowest priority */
	NVIC_InitStructure.NVIC_IRQChannel = ACCEL_INT1_IRQCHANNEL;
	...
	GPIO_InitStructure.GPIO_Pin = MAG_DRDY_GPIO_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(MAG_DRDY_GPIO_PORT, &GPIO_InitStructure);

	/* Connect EXTI Line to MAG_DRDY pin */
	GPIO_EXTILineConfig(MAG_DRDY_EXTI_PORT_SRC, MAG_DRDY_EXTI_PIN_SRC);

	/* Configure MAG_DRDY EXTI line */
	EXTI_InitStructure.EXTI_Line = MAG_DRDY_EXTI_LINE;
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;
	EXTI_Init(&EXTI_InitStructure);

	/* Enable and set MAG_DRDY Interrupt to the lowest priority */
	NVIC_InitStructure.NVIC_IRQChannel = MAG_DRDY_IRQCHANNEL;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x02;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x02;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
...
void lsm303dlhcConfig(void)
{
	lsm303dlhcIrqConfig();
   	...
   	FIFOModeEnable(FIFO_STREAM_MODE);
...
status_t GetSatusBit(u8_t statusBIT)
{
	...
	case STATUS_REG_ZYXOR:
		if (value &= STATUS_REG_ZYXOR
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_ZOR:
		if (value &= STATUS_REG_ZOR
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_YOR:
		if (value &= STATUS_REG_YOR
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_XOR:
		if (value &= STATUS_REG_XOR
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_ZYXDA:
		if (value &= STATUS_REG_ZYXDA
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_ZDA:
		if (value &= STATUS_REG_ZDA
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_YDA:
		if (value &= STATUS_REG_YDA
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
	case STATUS_REG_XDA:
		if (value &= STATUS_REG_XDA
			) return MEMS_SUCCESS;
		else
			return MEMS_ERROR;
...
// Read 3 mag values into user-provided buffer. No overrun checking is done.
void lsm303dlhcReadMag(int16_t *magData)
{
    uint8_t buf[6];
    i2cRead(MAG_I2C_ADDRESS, OUT_X_H_M | 0x80, 6, buf);
    magData[1] =  (int16_t)((buf[0] << 8) | buf[1]);	// X_H, X_L
    magData[2] = -(int16_t)((buf[2] << 8) | buf[3]);	// Z_H, Z_L
    magData[0] = -(int16_t)((buf[4] << 8) | buf[5]);	// Y_H, Y_L
}

...
void gps_navigate(void)
{
	...
	if (!flag(FLAG_ARMED))
		flagClear(FLAG_GPS_FIX_HOME);

	if (!flag(FLAG_GPS_FIX_HOME) && flag(FLAG_ARMED))
	...
	if (!flag(FLAG_GPSHOLD_MODE) && !flag(FLAG_GPSHOME_MODE)) return;
	...
	case NAV_MODE_WP:
		...
		if ((wp_distance <= cfg.gps_wp_radius) || check_missed_wp())
...
portTASK_FUNCTION_PROTO(navigateTask, pvParameters)
{
	...
	while (1)
    {
		...
		vTaskDelayUntil(&xLastWakeTime, 100); 	// Task cycle rate 10 Hz
#endif
		}
}
